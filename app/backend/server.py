...
@app.get("/api/icons")
async def get_icons(search: Optional[str] = None, category: Optional[str] = None, limit: int = 100):
    query = {}
...
async def create_icon(icon: IconCreate, email: str = Depends(verify_admin_token)):
    ...
    if existing:
        raise HTTPException(status_code=400, detail=f"Icon with name '{icon.name}' already exists.") 
      icon_doc = {
        "id": str(uuid.uuid4()),
...
.myicon svg { width: 100%; height: 100%; fill: currentColor; }
.myicon.colorful svg { fill: initial; }
"""
    for ic in icons:
        name = ic["name"]
        svg = ic["svg_code"].replace('"', "'").replace("\n", " ")
        css_content += f'\n.myicon-{name}::before {{ content: ""; display: inline-block; width: 1em; height: 1em; background-repeat: no-repeat; background-position: center; background-size: contain; background-image: url("data:image/svg+xml;utf8,{svg}"); }}\n'
...
async def health_check():
    return {"status": "healthy", "service": "glyphcraft-icon-library-backend"}
